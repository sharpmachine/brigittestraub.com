	    ...
	    function search(params) {
            ...
            xhr = $.post(ajaxurl, params, function(data) {
            	...
            	for (var i = 0, len = entries.length; i < len; ++i) {
            		...
            		var flash = "http://youtube.com/v/" + videoId + '?autoplay=1';
            		var toInsert = "http://youtube.com/v/" + videoId;
            		var mediaThumbnail = entries[i].media$group.media$thumbnail;
                    	...
                    	html += '<li class="search-result"><img class="search-result-img" src="' + thumb + '" /><div style="text-align: center; margin-top: 0;"><a href="' + largeThumb + '" class="view-larger">View larger</a> | <a href="' + flash + '" class="view" >View video</a> | <a href="' + toInsert + '" class="insert">Insert</a></div></li>';
            	}
            	
            	$("#search_results").html(html);
	            $(".view-larger").bind("click", function(e) {
	                e.preventDefault();
	                var $this = $(this);
	                var src = $this.attr("href");
	                $("#large_image").attr("src", src);
	                $("#insert_from_image").attr("href", $this.parent().find(".insert").attr("href"));
	                $("#search_wrapper").hide();
	            ...
	            $(".insert").bind("click", function(e) {
	                e.preventDefault();
	                var html = '[youtube v="' + videoId+ '"]';
//flashHtml($(this).attr("href"));
	                sendToEditor(html);
	            });      	
