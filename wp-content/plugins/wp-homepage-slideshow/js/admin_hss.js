...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ pp_upload_dir+"/"+file.name + '&task_hss=resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function pp_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task_hss=get_albums_table', function(res)
	{
		pp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function pp_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task_hss=get_albums_images_table&album_id='+album_id, function(res)
	{
		pp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function pp_submit_bulk()
{
	...
	if( task == 'bulk_delete_albums' || task == 'bulk_enable_albums' || task == 'bulk_disable_albums')
	{
		...
		__log(ids);
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
		jQuery.post('index.php', 'task_hss='+task+'&ids='+ids, function(res)
		{
				pp_refresh_albums_table();
				if( res.status == 'error')
				{
