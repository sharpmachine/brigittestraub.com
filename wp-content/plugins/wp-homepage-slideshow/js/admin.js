...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ hss_upload_dir+"/"+file.name + '&task=hss_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	hss_refresh_images_table(hss_current_album_id);
...
function hss_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=hss_get_albums_table', function(res)
	{
		hss_set_data_tables();
		if (res.status == 'ok') 
		{
...
function hss_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=hss_get_albums_images_table&album_id='+album_id, function(res)
	{
		hss_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'hss_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'hss_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'hss_reorder_image' )
		{
			hss_refresh_images_table(album_id);
		}
		if( task == 'hss_reorder_album' )
			hss_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function hss_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', hss_submit_bulk);
...
function hss_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'hss_bulk_delete_albums' || task == 'hss_bulk_enable_albums' || task == 'hss_bulk_disable_albums')
	{
	...
	else if( task == 'hss_bulk_delete_images' || task == 'hss_bulk_enable_images' || task == 'hss_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = hss_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: hss_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: hss_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: hss_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: hss_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				...
				upload_url: hss_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":hss_upload_dir, "secw":hss_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: hss_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.hss_delete_album').live('click', hss_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', hss_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', hss_disable);
