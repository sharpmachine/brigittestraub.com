...
if( !($album = $ghss->hss_get_album($album_id)) )
{
	die('Album id does not exists');
}
if( !($image = $ghss->get_image($image_id)) )
{
...
<?php endif;?>
	...
	do_action("hss_admin_head-$hook_suffix");
	do_action('hss_admin_head'); 
	?>
</head>
<body>
<div id="wpwrap">
		...
		<div id="wpbody"><!-- <div id="wpbody-content">  -->
			<div class="wrap" style="padding: 12px;">
				...
				<form id="edit_album_form" action="<?php print admin_url() ?>" method="post" enctype="multipart/form-data">
					...
					<tr>
						<td><label><?php _e('View more Link'); ?></label></td>
					...
					<tr>
						...
						<td>
							...
							<img src="<?php print hss_get_album_url($album_id) . '/thumb/' . $image['thumb']; ?>" />
						</td>
					</tr>
					</table>
					<p>
